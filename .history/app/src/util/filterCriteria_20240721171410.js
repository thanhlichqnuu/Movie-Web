...
const statusMap = computed(() =>
    Object.fromEntries(statusOptions.value.map(option => [option.value, option.text]))
  );

  const countryMap = computed(() =>
    Object.fromEntries(countryOptions.value.map(option => [option.value, option.text]))
  );

  const genreMap = computed(() =>
    Object.fromEntries(genreOptions.value.map(option => [option.value, option.text]))
  );


  return { statusOptions, countryOptions, genreOptions, statusMap, countryMap, genreMap };
}
