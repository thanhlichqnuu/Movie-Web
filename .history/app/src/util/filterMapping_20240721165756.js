...
export function getFilterText(query, t) {
  ...
  // Use direct access to computed values
  const statusMap = mapOptions(statusOptions);
  const countryMap = mapOptions(countryOptions);
  const genreMap = mapOptions(genreOptions);
