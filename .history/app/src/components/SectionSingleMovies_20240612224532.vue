...
const updateMetaTitle = () => useHead({ title: `Danh Sách Phim Lẻ Đầy Đủ Nhất | Tổng Hợp Những Phim Lẻ Hay | Phim Bộ Mới Nhất ${currentYear}` });
...
const prefetchNextPage = async (page) => {
  if (page < totalPages.value) {
    const cachedMovies = await get(`all_series_page_${page + 1}`);
    if (!cachedMovies) {
