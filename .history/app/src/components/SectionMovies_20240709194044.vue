...
const { data, mutate } = useSWRV(
  ...
  fetcher,
  {
    ...
    ttl: 0, // cache data stays forever
    onSuccess: (newData) => {
      if (newData) {
...
const prefetchNextPage = async (page) => {
  if (page < totalPages.value) {
    useSWRV(`https://ophim1.com/danh-sach/phim-moi-cap-nhat?page=${page + 1}`, fetcher, {
...
watch(data, (newData) => {
  if (newData) {
    ...
    totalPages.value = newData.pagination.totalPages;
    prefetchNextPage(currentPage.value);
    // Invalidate cache for the current page to remove old data
    mutate(false);
  }
});
</script>
