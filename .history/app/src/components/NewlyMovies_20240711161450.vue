...
const tabs = ref([
  { name: "Anime", key: "animes" },
  { name: movieLabel, key: "singleMovies" },
  { name: seriesLabel, key: "seriesMovies" },
  { name: "TV Show", key: "tvShows" },
]);

const router = useRouter();
const currentTab = ref(tabs.value[0].name);
const movies = computed(() => movieStore.movies[tabs.value.find(tab => tab.name === currentTab.value)?.key] || []);
