...
const getMovies = async (page) => {
  const cacheKey = `all_moviessss_page_${page}`;
  const cachedMovies = await get(cacheKey);
  const now = Date.now();

  if (cachedMovies && (now - cachedMovies.timestamp < CACHE_EXPIRATION_MS)) {
...
const prefetchNextPage = async (page) => {
  if (page < totalPages.value) {
    const cachedMovies = await get(`moviessss_page_${page + 1}`);
    if (!cachedMovies) {
      getMovies(page + 1);
    }
  }
