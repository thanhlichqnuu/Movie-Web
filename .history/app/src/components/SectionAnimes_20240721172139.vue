...
const listLabel = computed(() => t("seriesMovie"));

const filterText = computed(() => {
  const filters = [];
  if (route.query.year) filters.push(`${t("releaseYear")}`);
  if (route.query.category)
    filters.push(
      `${t("genre")} ${
        genreMap[route.query.category] 
      }`
    );
  if (route.query.status)
    filters.push(
      `${t("status")} ${
        statusMap[route.query.status] 
      }`
    );
  if (route.query.country)
    filters.push(
      `${t("country")} ${
        countryMap[route.query.country]
      }`
    );
  return filters.length ? ` ${route.name} ${filters.join(", ")}` : "";
});
