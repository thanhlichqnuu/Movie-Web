...
const isTablet = computed(() => windowWidth.value < 640);
...
const prefetchNextPage = async (page) => {
  if (page < totalPages.value) {
    const cachedMovies = await get(`all_single_page_${page + 1}`);
    if (!cachedMovies) {
