...
<template>
  <v-main>
    <div>
      <h3 class="d-flex justify-start animate-charcter">{{ $t('lastUpdateLabel') }}</h3>
      <div class="line" />
    </div>
    
    <v-tabs v-model="currentTab">
      <v-tab class="d-flex justify-center" v-for="tab in tabs" :key="tab.name" @click="changeTab(tab)" class="tab-active">
        {{ tab.name }}
        <div v-if="tab.isActive" :class="themeStore.isDark ? 'tab-indicator-black' : 'tab-indicator-white'" />
      </v-tab>
    </v-tabs>
