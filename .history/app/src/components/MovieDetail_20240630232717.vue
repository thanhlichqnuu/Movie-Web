...
const getMovieDetail = async (slugMovie) => {
  try {
    ...
    toast.error("Movie detail is currently unavailable!");
  }
};
...
const loadTrailer = () => {
  if (!movie.value.movie?.trailer_url) {
    toast.error("Trailer is currently unavailable!");
    return;
  }
...
const loadMovie = () => {
  ...
  if (!first_episode) {
    toast.error("Episodes is currently unavailable!");
    return;
  }

  if (!first_episode.slug) {
    toast.error("Episode slug is missing!");
    return;
  }
              ...
              <div>
                ...
                <div>
                  ...
                  <v-card-action class="ml-4">
                    <v-btn
                    v-show="first_episode && first_episode.slug"
                      color="light-blue-darken-1"
                      height="40"
                      @click="loadMovie"
                      class="mr-3"
