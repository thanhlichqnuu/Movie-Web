...
const micAccessDenied = ref(false);
let recognition;
...
<template>
  <div>
    <v-text-field
      ...
      single-line
    ></v-text-field>
    <v-btn icon @click="toggleVoiceSearch">
      <v-icon>{{ isListening ? 'mdi-microphone-off' : 'mdi-microphone' }}</v-icon>
    </v-btn>
    <v-alert v-if="micAccessDenied" type="error" dismissible>
