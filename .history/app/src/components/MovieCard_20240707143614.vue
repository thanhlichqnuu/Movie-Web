...
import { compressImage } from '@/utils/compressImage'; // Adjust the path as necessary
...
onMounted(async () => {
  try {
    const originalUrl = `https://img.ophim.live/uploads/movies/${props.movie.thumb_url}`;
    compressedThumbUrl.value = await compressImage(originalUrl);
  } catch (error) {
...
<template>
  <v-card @click="navigateToDetail">
    <v-img
      max-height="250"
      :src="compressedThumbUrl || `https://img.ophim.live/uploads/movies/${movie.thumb_url}`"
      alt="thumbnail"
      loading="lazy"
