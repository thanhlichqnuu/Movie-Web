...
onMounted(() => {
  currentTab.value = tabs.value[0].name; // Set the initial tab value
  const initialTab = tabs.value.find(tab => tab.name === currentTab.value);
  if (initialTab) {
    mutate(initialTab.api);
  } else {
    console.error('Initial tab not found:', currentTab.value);
  }
});
