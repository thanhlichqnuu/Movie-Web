...
const getMoviePlay = async (slugMovie) => {
  try {
    const { data } = await axios.get(`https://apii.online/apii/phim/${slugMovie}`);
    movie.value = data;
    episodes.value = data.episodes[0].server_data;
    serverName.value = data.episodes[0].server_name;
    serverNames.value = data.episodes.map(ep => ep.server_name);
  } catch {
...
const changeServer = async (server) => {
  const selectedEpisode = movie.value.episodes.find(ep => ep.server_name === server);
  if (selectedEpisode) {
    episodes.value = selectedEpisode.server_data;
    serverName.value = selectedEpisode.server_name;
    selectedEpisode(currentEpisode.value)
...
const setupPlayer = async () => {
  ...
  const isMobile = /iPhone|iPad|Android|Windows Phone|BlackBerry/i.test(navigator.userAgent)
      ...
      <v-col :cols="isSmallTabletAndMobile ? 12 : 7" class="mt-10">
        <div class="mb-3">
          <v-btn
            ...
            :color="server === serverName ? 'primary' : 'default'"
            class="mx-1 my-1"
          >
          <svg
