...
const getMoviePlay = async (slugMovie) => {
  try {
    ...
    episodes.value = data.reversedEpisodes[0].server_data;
    serverNames.value = data.reversedEpisodes.map((episode) => episode.server_name);
    serverName.value = data.reversedEpisodes[0].server_name;
  } catch {
...
const changeServer = async (server) => {
  ...
  if (selectedServer) {
    if (
      !selectedServer.server_data[0]?.slug
    ) {
      const { toast } = await import("vue3-toastify");
