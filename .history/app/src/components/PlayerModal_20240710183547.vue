...
const setupPlayer = async () => {
  ...
  const isMobile =
    ...
    player.value.addEventListener("dblclick", (event) => {
      const rect = player.value.getBoundingClientRect();
      const clickX = event.clientX - rect.left;
      const clickPositionRatio = clickX / rect.width;

      if (clickPositionRatio < 0.3) {
        player.value.currentTime = Math.max(0, videoElement.currentTime - 5);
        videoElement.play();
