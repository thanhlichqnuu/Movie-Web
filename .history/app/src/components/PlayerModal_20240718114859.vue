...
const changeServer = async (server) => {
  const selectedServer = movie.value.episodes.find(episode => episode.server_name === server);
  if (selectedServer) {
    const currentEpisodeSlug = currentEpisode.value.slug;
    episodes.value = selectedServer.server_data;
    serverName.value = selectedServer.server_name;

    const matchingEpisode = episodes.value.find(episode => episode.slug === currentEpisodeSlug);
    if (matchingEpisode) {
      await selectEpisode(matchingEpisode);
    } else {
...
const updateMetaTitle = (newEpisode) => {
  if (newEpisode?.name) {
    document.title = newEpisode.slug.includes("tap")
...
watch(currentEpisode, (newEpisode) => {
  updateDocumentTitle(newEpisode);
}, { immediate: true });
