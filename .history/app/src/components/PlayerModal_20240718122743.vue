...
const changeServer = (server) => {
  ...
  if (selectedServer) {
    const currentSlug = currentEpisode.value.slug;
    episodes.value = selectedServer.server_data;
    serverName.value = selectedServer.server_name;
    const sameEpisode = episodes.value.find(
      (episode) => episode.slug === route.params.slu
    );
    if (sameEpisode) {
      selectEpisode(sameEpisode);
    } else {
      selectEpisode(episodes.value[0]); // Fallback to the first episode if the same episode is not found
    }
  }
};
