...
const url = ref(`https://apii.online/apii/danh-sach?type=hoathinh&page=${currentPage.value}&limit=12`)

const urls = computed(() => {
  if (route.query.year) url.value += `&year=${route.query.year}`;
  if (route.query.status) url.value += `&status=${route.query.status}`;
  if (route.query.country) url.value += `&country=${route.query.country}`;
  if (route.query.category) url.value += `&category=${route.query.category}`;
  return url.value;
});
