...
const { filterLabels } = useFilterCriteria();
...
const filterText = computed(() => {
  ...
  if (route.query.category)
    filters.push(`${t("genre")} ${filterLabels.genre[route.query.category] || route.query.category}`);
  if (route.query.year)
    filters.push(`${t("releaseYear")} ${route.query.year}`);
  if (route.query.status)
    filters.push(`${t("status")} ${filterLabels.status[route.query.status] || route.query.status}`);
  if (route.query.country)
    filters.push(`${t("country")} ${filterLabels.country[route.query.country] || route.query.country}`);
  return filters.length
    ? `Danh sách ${route.name} ${filters.join(", ")}`
    : "";
});
