...
const setupPlayer = async () => {
  ...
  player.value = new Artplayer({
    ...
    controls: [
      {
        position: 'left',
        html: '<button class="art-control art-control-skip" title="Skip back 10s">⏪ -10s</button>',
        click: () => player.value.currentTime(player.value.currentTime - 10),
      },
      {
        position: 'right',
        html: '<button class="art-control art-control-skip" title="Skip forward 10s">⏩ +10s</button>',
        click: () => player.value.seek(player.value.currentTime + 10),
      }
    ],
  });
  ...
  if (isMobile) {
    const video = player.value.template.$video;
