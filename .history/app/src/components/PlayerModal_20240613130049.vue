...
const updateMetaTitle = () => {
  if (movie.value && currentEpisode.value) {
    const episodeName = currentEpisode.value.name;
    const titleSuffix = episodeName.includes("Tập") ? episodeName : `Tập ${episodeName}`;
    useHead({
      title: `${movie.value.movie.name} - ${titleSuffix}`,
    });
  }
};
...
const selectEpisode = async (episode) => {
  ...
  const formattedSlug = episodeSlug.includes("tap") ? episodeSlug : `tap-${episodeSlug}`;
  const formattedName = episodeName.includes("Tập") ? episodeName : `Tập ${episodeName}`;
  
  const routeParams = {
    name: "player-route",
    params: {
      slugMovie: route.params.slugMovie,
      slugEpisode: formattedSlug
    }
  };
