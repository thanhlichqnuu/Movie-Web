  ...
  <v-main>
    <v-tabs v-model="currentTab">
      <v-tab class="text-subtitle-1" v-for="tab in tabs" :key="tab.name" @click="changeTab(tab)">
        {{ tab.name }}
        <div v-if="currentTab === tab.name" class="active-tab-line"></div>
      </v-tab>
    </v-tabs>
...
.active-tab-line {
  ...
  position: absolute;
  bottom: -1px; /* Để nó nằm bên dưới tab */
  transition: all 0.3s ease; /* Hiệu ứng chuyển động */
  z-index: 1; /* Đảm bảo đường line gạch chân hiển thị phía trên cùng */
}

.active-tab-line.active {
