...
const updateMetaTitle = () => movie.value && useHead({ title: `${movie.value.movie.name} - ${movie.value.movie.origin_name}` });

const getMovieDetail = async (slugMovie) => {
  try {
    const cacheKey = `movie_detail_${slugMovie}`;
    const cachedMovie = await get(cacheKey) || (await axios.get(`https://ophim1.com/phim/${slugMovie}`)).data;
    cachedMovie.movie.content = sanitizeHtml(cachedMovie.movie.content, { allowedTags: [], allowedAttributes: {} });
    movie.value = cachedMovie;
    set(cacheKey, cachedMovie);
  } catch {
...
watch(() => route.params.slugMovie, async (newSlugMovie) => {
  movie.value = null;
...
const loadTrailer = () => {
  if (!movie.value.movie?.trailer_url) return toast.error("Trailer hiện không có sẵn!");
  isShowTrailer.value = true;
};

const loadMovie = () => {
  const firstEpisode = movie.value.episodes?.[0]?.server_data?.[0];
  if (!firstEpisode) return toast.error("Tập phim hiện không có sẵn!");
  router.push({ name: "player-route", params: { slugMovie: movie.value.movie.slug, slugEpisode: firstEpisode.slug } });
};
