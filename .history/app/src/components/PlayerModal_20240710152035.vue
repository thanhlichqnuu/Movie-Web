...
const setupPlayer = async () => {
  ...
  let urlToUse = currentEpisode.value.link_m3u8;
  let typeToUse = 'm3u8';

  try {
    await axios.head(currentEpisode.value.link_m3u8);
  } catch (error) {
    if (error.response && error.response.status === 403) {
      urlToUse = currentEpisode.value.link_embed;
      typeToUse = 'embed';
    }
  }
  ...
  if (typeToUse === 'm3u8') {
    ...
    player.value = null; // No need to keep Artplayer instance
    videoPlayer.value.innerHTML = `<iframe src="${urlToUse}" width="100%" height="100%" frameborder="0" allowfullscreen></iframe>`;
  }
      ...
      <v-col cols="12" class="d-flex justify-center">
        <div class="w-100">
          <div ref="videoPlayer" style="width: 100%; height: 75vh" />
            ...
            <div v-if="player"></div>
            <div v-else-if="currentEpisode.value && currentEpisode.value.link_embed">
              <iframe
