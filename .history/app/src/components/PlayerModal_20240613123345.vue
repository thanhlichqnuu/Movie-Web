...
import { addTapPrefixIfNeeded } from "@/utils";  // Import hàm tiện ích
...
const updateMetaTitle = () => {
  if (movie.value && currentEpisode.value) {
    useHead({
      title: `${movie.value.name} - ${addTapPrefixIfNeeded(currentEpisode.value.name)}`,
    });
  }
};
...
const currentEpisode = computed(
  () =>
    episodes.value.find(
      (episode) => addTapPrefixIfNeeded(episode.slug) === addTapPrefixIfNeeded(route.params.slugEpisode)
    ) || episodes.value[0]
);

const selectEpisode = async (episode) => {
  const episodeSlug = addTapPrefixIfNeeded(episode.slug);
  router.push({
    name: "player-route",
    params: { slugMovie: route.params.slugMovie, slugEpisode: episodeSlug },
  });
  await nextTick();
...
const updateEpisodeFromRoute = () => {
  const episodeSlug = addTapPrefixIfNeeded(route.params.slugEpisode);
  const episode = episodes.value.find(
    (ep) => addTapPrefixIfNeeded(ep.slug) === episodeSlug
  );
  if (episode) {
