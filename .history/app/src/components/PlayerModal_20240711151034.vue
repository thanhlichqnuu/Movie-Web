...
const setupPlayer = async () => {
  ...
  player.value = new Artplayer({
    ...
    type: "m3u8",
    setting: !isSmallTabletAndMobile.value,
    fullscreen: true,
    fullscreenWeb: !isSmallTabletAndMobile.value,
    playbackRate: true,
    aspectRatio: true,
    pip: true,
    controls: [
  ...
  player.value.on('fullscreen', (state) => {
    if (state) {
      player.value.setting = !isSmallTabletAndMobile.value;
    } else {
      player.value.setting = true;
    }
  });
  player.value.on('fullscreen', (state) => {
    if (state && isSmallTabletAndMobile.value && window.screen.orientation.type.includes('landscape')) {
      player.value.setting = true;
    } else {
      player.value.setting = !isSmallTabletAndMobile.value;
    }
...
// Sự kiện lắng nghe thay đổi orientation
window.addEventListener('orientationchange', () => {
  if (player.value) {
    if (document.fullscreenElement && isSmallTabletAndMobile.value && window.screen.orientation.type.includes('landscape')) {
      player.value.setting = true;
    } else {
      player.value.setting = !isSmallTabletAndMobile.value;
    }
