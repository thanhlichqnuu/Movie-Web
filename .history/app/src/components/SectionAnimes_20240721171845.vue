...
const filterText = computed(() => {
  ...
  if (route.query.category)
    filters.push(
      `${t("genre")} ${
        genreMap.value[route.query.category] || route.query.category
      }`
    );
  ...
  if (route.query.status)
    filters.push(
      `${t("status")} ${
        statusMap.value[route.query.status] || route.query.status
      }`
    );
  if (route.query.country)
    filters.push(
      `${t("country")} ${
        countryMap.value[route.query.country] || route.query.country
      }`
    );
  return filters.length ? `Danh sách ${route.name} ${filters.join(", ")}` : "";
});
