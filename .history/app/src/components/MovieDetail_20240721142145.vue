...
const fetcher = async (url) => {
  try {
    ...
    data.movie.content = data.movie.content
      .replace(/<\/?[^>]+(>|$)/g, "")  
      .replace(/&quot;/g, "")
      .replace(/&#39;/g, "") 
      .replace(/&nbsp;/g, "");     
    ...
    <v-card v-if="movie">
      <v-row>
        <v-col cols="12" class="background-container d-flex">
          ...
          <template>
  <v-col
    :cols="isSmallTabletAndMobile ? 12 : 6"
    :sm="!isSmallTabletAndMobile ? (isSmallTabletAndMobile ? 12 : 8) : null"
    class="d-flex"
    :class="!isSmallTabletAndMobile ? 'justify-center align-center' : 'flex-column'"
  >
    ...
    <div v-else>
      <div>
        <v-card-title class="text_green text-h5">{{ movie.movie.name }}</v-card-title>
        <v-card-title class="text-subtitle-1 mt-n3">{{ movie.movie.origin_name }}</v-card-title>
      </div>
      <v-img :src="movie.movie.thumb_url" height="300px" class="mb-3" />
    </div>
    <div class="mt-11" v-if="!isSmallTabletAndMobile">
      <v-card-title class="text_green text-h5">{{ movie.movie.name }}</v-card-title>
      <v-card-title class="mt-n3" style="font-size: 16px">{{ movie.movie.origin_name }}</v-card-title>
      <v-card-text class="text_dim_gray">{{ movie.movie.content }}</v-card-text>
    </div>
    <div v-else>
      <v-card-text class="text_dim_gray">{{ movie.movie.content }}</v-card-text>
    </div>
    <v-card-action :class="!isSmallTabletAndMobile ? 'mb-15 ml-4' : 'd-flex ml-4'">
      <v-btn
