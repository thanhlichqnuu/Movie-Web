<template>
    <v-breadcrumbs>
      <v-breadcrumbs-item v-for="(crumb, index) in breadcrumbs" :key="index" :to="crumb.to">
        {{ crumb.text }}
      </v-breadcrumbs-item>
      <v-breadcrumbs-divider v-if="index < breadcrumbs.length - 1" :key="'divider-' + index">/</v-breadcrumbs-divider>
    </v-breadcrumbs>
  </template>
