...
const updateMetaTitle = () => useHead({ title: `Danh Sách Anime Đầy Đủ Nhất | Tổng Hợp Những Anime Hay | Anim Mới Nhất ${currentYear}` });
...
const getMovies = async (page) => {
  try {
    ...
    if (data) {
      movies.value = cachedMovies.data.data.items;
      totalPages.value = Math.ceil(cachedMovies.data.params.pagination.totalItems / cachedMovies.data.params.pagination.totalItemsPerPage) 
    } else {
      const { data } = await axios.get(`https://ophim1.com/v1/api/danh-sach/hoat-hinh?page=${page}`);
