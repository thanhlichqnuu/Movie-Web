...
const getMovies = async (page) => {
  try {
    ...
    const cachedMovies = useLocalStorage(cacheKey, null);
    if (cachedMovies.value) {
      movies.value = cachedMovies.value.items;
      totalPages.value = cachedMovies.value.pagination.totalPages;
    } else {
...
const prefetchNextPage = async (page) => {
  if (page < totalPages.value) {
    const nextPageCacheKey = `all_movies_page_${page + 1}`;
    const cachedMovies = useLocalStorage(nextPageCacheKey, null);
    if (!cachedMovies.value) {
      getMovies(page + 1);
    }
  }
