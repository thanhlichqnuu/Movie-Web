...
const handleSearch = async (keyword) => {
  ...
  try {
    ...
    toast.error("Movie information is currently unavailable");
  } finally {
...
watch(keyword, (newKeyword) => debounceHandleSearch(newKeyword));

const navigateToDetail = (slugMovie) => {
  router.push({ name: "detail-route", params: { slugMovie } });
  emit("close-modal");
};
...
<template>
  <v-card max-width="600" class="pa-3 mx-auto">
    <v-text-field
      v-model="keyword"
      :loading="isLoading"
