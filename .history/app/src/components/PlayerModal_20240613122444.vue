...
const addTapPrefixIfNeeded = (text) => {
  return /^\d+$/.test(text) ? `Tập ${text}` : text;
};

const updateMetaTitle = () => {
  if (movie.value && currentEpisode.value) {
    const episodeName = addTapPrefixIfNeeded(currentEpisode.value.name);
    useHead({
      title: `${movie.value.movie.name} - ${episodeName}`,
    });
  }
};
        ...
        <v-col cols="12" class="mt-10 mr-6">
          <div class="d-flex justify-start flex-wrap">
            <v-btn
              ...
              class="mx-1 my-1"
            >
              {{ addTapPrefixIfNeeded(episode.name) }}
            </v-btn>
          </div>
        </v-col>
      </v-row>
