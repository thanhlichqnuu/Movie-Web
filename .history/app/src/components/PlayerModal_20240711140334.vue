...
const setupPlayer = async () => {
  ...
  const skipBackwardSvg = `
    <svg width="20" height="20" viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
      <path d="M10 12L18 18V6L10 12ZM6 18V6H4V18H6Z"/>
    </svg>`;

  const skipForwardSvg = `
    <svg width="20" height="20" viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
      <path d="M14 12L6 6V18L14 12ZM18 6V18H20V6H18Z"/>
    </svg>`;
  ...
  player.value = new Artplayer({
    ...
    controls: [
      {
        position: 'right',
        html: `<svg fill="white" width="20" height="20">
                 <use :href="${skipBackwardSvg}"></use>
               </svg>`,
        click: () => player.value.currentTime = player.value.currentTime - 10,
      },
      {
        position: 'right',
        html: `<img src="${skipForwardSvg}" alt="Forward" style="width: 20px; height: 20px;">`,
        click: () => player.value.currentTime = player.value.currentTime + 10,
      }
    ],
  });
