...
const getMovieDetail = async (slugMovie) => {
  try {
    ...
    if (cachedMovie) return movie.value = cachedMovie;
    const { data } = await axios.get(`https://ophim1.com/phim/${slugMovie}`);
    data.movie.content = sanitizeHtml(data.movie.content, { allowedTags: [], allowedAttributes: {} });
    movie.value = data;
    set(cacheKey, data);
  } catch {
...
const loadMovie = () => {
  ...
  if (!first_episode) return toast.error("Tập phim hiện không có sẵn!");
  router.push({ name: "player-route", params: { slugMovie: movie.value.movie.slug, slugEpisode: first_episode.slug } });
};
