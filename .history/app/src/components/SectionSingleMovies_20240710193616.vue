...
const fetcher = async (url) => {
  try {
    ...
    toast.error("Movies list is currently unavailable!");
  }
};

const { data } = useSWRV(
  () => `https://ophim1.com/v1/api/danh-sach/phim-le?page=${currentPage.value}`,
  fetcher,
  {
...
const prefetchNextPage = async (page) => {
  if (page < totalPages.value) {
    const nextPageUrl = `https://ophim1.com/v1/api/danh-sach/phim-le?page=${page + 1}`;
    try {
