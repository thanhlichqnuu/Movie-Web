...
const filterText = computed(() => {
  ...
  if (route.query.category)
    filters.push(
      `${t("genre")} ${
        genreMap.value[route.query.category] 
      }`
    );
  if (route.query.status)
    filters.push(
      `${t("status")} ${
        statusMap.value[route.query.status] 
      }`
    );
  if (route.query.country)
    filters.push(
      `${t("country")} ${
        countryMap.value[route.query.country]
      }`
    );
  return filters.length ? `listLabel ${route.name} ${filters.join(", ")}` : "";
});
