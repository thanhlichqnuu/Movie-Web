...
const changeServer = async (server) => {
  ...
  if (selectedServer) {
    // Find the corresponding episode in the new server
    const correspondingEpisode = selectedServer.server_data.find(
      (episode) => episode.slug === currentEpisode.value.slug
    );

    episodes.value = selectedServer.server_data;
    serverName.value = selectedServer.server_name;
    
    // Update the route with the new episode's slug
    if (correspondingEpisode) {
      await router.push({
        name: "Player",
        params: { slugEpisode: correspondingEpisode.slug },
      });
      await nextTick();
      setupPlayer();
    } else {
