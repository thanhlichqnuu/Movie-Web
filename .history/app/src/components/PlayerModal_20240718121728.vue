...
const changeServer = (server) => {
  ...
  if (selectedServer) {
    ...
    // Find the corresponding episode in the new server's episodes
    const currentEpisodeSlug = route.params.slugEpisode;
    const correspondingEpisode = episodes.value.find(
      (episode) => episode.slug === currentEpisodeSlug
    );
    
    if (correspondingEpisode) {
      selectEpisode(correspondingEpisode);
    } else {
