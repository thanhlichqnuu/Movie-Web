...
const getMovieDetail = async (slugMovie) => {
  try {
    ...
    movie.value = data;
    first_episode.value = movie.value?.episodes?.[0]?.server_data?.[0];
    set(cacheKey, data);
  } catch {
...
const loadMovie = () => {
  router.push({
    ...
    params: {
      ...
      slugEpisode: first_episode.value.slug,
    },
  });
};

const episodeAvailable = computed(() => {
  return first_episode.value && first_episode.value.slug;
});
