...
const changeServer = async (server) => {
  ...
  if (selectedServer) {
    if (!selectedServer || !selectedServer.server_data || selectedServer.server_data.length === 0) {
    const { toast } = await import("vue3-toastify");
    toast.error("The selected server has no episodes available.");
    return;
  }

  if (!selectedServer.server_data[0].slug) {
    const { toast } = await import("vue3-toastify");
    toast.error("The first episode of the selected server has an empty slug.");
    return;
  }
