...
const filterText = computed(() => {
  const filters = [];
  if (route.query.category) filters.push(`${t('genre')}: ${route.query.category}`);
  if (route.query.year) filters.push(`${t('releaseYear')}: ${route.query.year}`);
  if (route.query.status) filters.push(`${t('status')}: ${route.query.status}`);
  if (route.query.country) filters.push(`${t('country')}: ${route.query.country}`);
  return filters.length ? `Danh sách "${route.name}" thuộc ${filters.join(', ')}` : '';
});
  ...
  <v-container fluid class="mt-n10">
    <v-row>
      <v-col :cols="isTabletAndMobile ? 12 : 8">
        <span class="filter-text" v-if="filterText">{{ filterText }}</span>
        <movie-list
          :movies="movies"
