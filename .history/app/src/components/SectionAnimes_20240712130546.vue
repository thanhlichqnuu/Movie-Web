...
const isPrefetching = ref(false);  // Add this flag
...
const prefetchNextPage = async (page) => {
  if (page < totalPages.value && !isPrefetching.value) {
    isPrefetching.value = true;  // Set the flag
    const nextPageUrl = `https://ophim1.com/v1/api/danh-sach/hoat-hinh?page=${page + 1}`;
    try {
      ...
      isPrefetching.value = false;  // Clear the flag
    }
  }
};
